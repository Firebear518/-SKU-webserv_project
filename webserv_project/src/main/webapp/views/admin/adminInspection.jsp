...
<%
    ...
    String DB_URL  = "jdbc:mysql://localhost:3306/sku_webserv_db?allowPublicKeyRetrieval=true&useSSL=false&serverTimezone=Asia/Seoul&characterEncoding=UTF-8";
    String DB_USER = "root";
    String DB_PASS = "root";
...
<%
    try {
        ...
        String sql = "SELECT a.auction_id, p.title, p.seller_id, a.highest_bidder_id, a.current_price, a.status, a.reject_reason " +
        ...
        // 🎯 [완료 탭 필터 확장] 기존 완료 외에 추가된 최종/진행 상태도 완료 탭에 함께 걸러지도록 처리
        else if ("완료".equals(filter)) sql += " AND (a.status = '검수완료' OR a.status = '검수 완료' OR a.status = '배송중' OR a.status = '반품 진행중')";
        ...
        while (rs.next()) {
            ...
            else if ("검수완료".equals(status) || "검수 완료".equals(status)) { badgeClass = "bg-success text-white"; badgeText = "검수 완료"; }
            ...
            else if ("불합격(인수)".equals(status)) { badgeClass = "bg-dark text-white"; badgeText = "불합격(인수)"; }
            else if ("불합격(인수 거부)".equals(status)) { badgeClass = "bg-secondary text-white"; badgeText = "불합격(인수 거부)"; }
            else if ("반품 진행중".equals(status)) { badgeClass = "bg-danger text-white"; badgeText = "반품 진행중"; }
%>
                        <tr>
...
<%
            } else if ("불합격(인수)".equals(status)) {
%>
                                <button type="button" class="btn btn-success btn-sm fw-bold shadow-sm" onclick="updateStatus(<%= auctionId %>, '검수 완료')">
                                    <i class="bi bi-box-seam-fill"></i> 인수 확정
                                </button>
<%
            } else if ("검수완료".equals(status) || "검수 완료".equals(status) || "배송중".equals(status)) {
%>
                                <span class="text-muted small fst-italic"><i class="bi bi-truck"></i> 낙찰자에게 배송중</span>
<%
            } else if ("불합격(인수 거부)".equals(status)) {
%>
                                <button type="button" class="btn btn-danger btn-sm fw-bold shadow-sm" onclick="updateStatus(<%= auctionId %>, '반품 진행중')">
                                    <i class="bi bi-arrow-return-left"></i> 반품
                                </button>
<%
            } else if ("반품 진행중".equals(status)) {
%>
                                <span class="text-muted small fst-italic"><i class="bi bi-truck"></i> 판매자에게 반품 중</span>
<%
            } else {
%>
